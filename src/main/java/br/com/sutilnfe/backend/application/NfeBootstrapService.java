...
public class NfeBootstrapService {
    ...
    public void init() {
        try {
            ...
            Files.createDirectories(paths.jsons());
            Files.createDirectories(paths.erros());

            log.info("✅ Estrutura de diretórios criada com sucesso");
            log.info("   Não processadas: {}", paths.naoProcessadas());
            log.info("   Processadas: {}", paths.processadas());
            log.info("   JSONs: {}", paths.jsons());
